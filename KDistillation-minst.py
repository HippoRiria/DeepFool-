...
for epoch in range(num_epochs):
	...
	print('Train loss: {:.6f}, acc: {:.6f}'.format(train_loss / len_train_data, train_acc / len_train_data))
	## 保存模型参数
	torch.save (student_with_teacher.state_dict (), './student.pth')
	eval_loss = .0
	eval_acc = .0
	...
	with torch.no_grad():
		for batch_x, batch_y in tqdm(test_loader):
			...
			batch_y_teacher = torch.softmax(teacher(batch_x) / temperature, dim=1).detach()
			out_result = torch.softmax(out, dim=1)
			loss = (1 - loss_lambda) * criterion[0](out_result, batch_y) + loss_lambda * temperature ** 2 * criterion[1](torch.softmax(out / temperature, dim=1), batch_y_teacher)

			eval_loss += loss.data.item()
			eval_acc += (torch.max(out_result, 1)[1] == batch_y).sum().data.item()
		print('Ori_Eval loss: {:.6f}, Ori_acc: {:.6f}'.format(eval_loss / len_test_data, eval_acc / len_test_data))

...
def deepfool_t(image, net, num_classes, overshoot=0.02, max_iter=50):#图片、网络参数、分类数、最后添加的小扰动、最大迭代次数
...
#初始化
    ...
    I = (np.array(f_image)).flatten().argsort()[::-1]#argsort()返回数组从小到大的索引值，逆置后flatten()返回一个折叠成一维的数组
    ...
    k_i = label#标签
    temp1=fs[0,I[0]].float()
    print('分类器1的值:',temp1)
    print('分类器2的值:',fs[0,I[1]])
    print('分类器3的值:',fs[0,I[999]])
    print('原标签:',k_i)
    t=10
    print('所选择的分类为：',t)
    ...
    while k_i != t and loop_i < max_iter:#进入循环，开始叠加扰动。当分类器标签不变时以及迭代未超过最大迭代次数时
        ...
        for k in range(1, num_classes):#多分类常规情况
            if I[k]==t:
                ...
                pert = abs(f_k)/np.linalg.norm(w.flatten())#求该分类器的扰动，绝对值
